...
public:
    ...
    static Scene buildCameraLensGraph() {
        NSBundle * bundle = [NSBundle mainBundle];
        NSString * path = [bundle pathForResource:@"Camera_Lens" ofType:@"usdz"];
        ModelLoader loader;
        SceneGraph loadedGraph = loader.load([path UTF8String]);
        std::unique_ptr<BaseNode> & root = loadedGraph.root;
        Node<TransformNodeContents> * transformNode = (Node<TransformNodeContents> *)(root.get());
    ...
    static Scene buildGraph() {
        ...
        g.environment = std::make_shared<RGBAUInt8Bitmap>(*BitmapLoader::load("kiara_1_dawn.jpg"));

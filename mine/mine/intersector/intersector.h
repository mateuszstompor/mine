...
namespace mine {
    class Intersector {
    public:
        std::optional<RayIntersection> closestIntersection(mine::Scene const & s,
                                                           Ray const & r) {
            std::optional<RayIntersection> closest = std::nullopt;
            for (mine::OmniLight const & sObject : s.omnilights) {
    ...
    //            simd::float2 uv = sIntersector.getTextureCoordinates(point, sphere);
            ...
            for (mine::SphereObject const & sObject : s.spheres) {
            ...
            for (mine::TriangleObject const & tObject : s.triangles) {

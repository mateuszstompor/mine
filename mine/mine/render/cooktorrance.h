...
namespace mine {
    inline float distributionGGX(float alpha, const simd::float3& n, const simd::float3& h) {
    ...
    inline simd::float3 fresnelSchlick(const simd::float3& f0,
                                       const simd::float3& v,
                                       const simd::float3& h) {
    ...
    inline float geometrySchlickGGX(const simd::float3& v, const simd::float3& n, float k) {
        float nDotV = std::max(simd::dot(n, v), 0.0f);
        return nDotV / (nDotV * (1.0f - k) + k);
    }

    inline float geometrySmith(const simd::float3& v, const simd::float3& n, const simd::float3& l, float k) {
        float gV = geometrySchlickGGX(v, n, k);
        float gL = geometrySchlickGGX(l, n, k);
        return gV * gL;
    }

    inline simd::float3 cookTorrance(const simd::float3& v,
                                     const simd::float3& n,
                                     const simd::float3& l,
